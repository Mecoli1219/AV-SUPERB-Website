...
import { SubmissionInfo } from "../../hooks";
import { DATA_VALUES, KEY_NAME, SHOWN_VALUES } from "../../constants/profile";
import HoverDescription from "./HoverDescription";

export const Table = ({ allData, name, setShowOnLeaderboard, downloadPreviousUpload }: {
    allData: SubmissionInfo[],
    name: string,
    setShowOnLeaderboard: (submission_id: string) => Promise<void>,
    downloadPreviousUpload: (submission_id: string) => Promise<void>
}) => {
    const [sortKey, setSortKey] = useState<[keyof SubmissionInfo, boolean] | null>(null);
    const [shownData, setShownData] = useState<SubmissionInfo[]>([]);
    const displayRules = (data: SubmissionInfo) => {
        return true
    }
    ...
    const sortData = (data: SubmissionInfo[]) => {
        if (sortKey) {
    ...
    const changeSortKey = (key: keyof SubmissionInfo, direction: number) => {
    ...
    const parseData = (submission: SubmissionInfo, key: keyof SubmissionInfo) => {
                            ...
                            shownData.map((submission, index) => {
                                return <tr key={index} className="bg-transparent border-b">
                                    <th scope="col" className="px-6 py-4 text-gray-900 whitespace-nowrap bg-gray-50 border-b border-r sticky left-0">
                                        {new Date((submission[KEY_NAME] as Date).getTime() + (720 - localTimeZoneOffsetMinutes) * 60 * 1000).toLocaleString()}
                                    </th>
                                    {
